...
for entry in data:
    ...
    start = dateutil.parser.parse(entry["start"])
    stop = dateutil.parser.parse(entry["stop"])
...
def format_delta(start, stop):
    ...
    truncated_start = start.replace(second=0, microsecond=0)
    truncated_stop = stop.replace(second=0, microsecond=0)
    delta = truncated_stop - truncated_start
